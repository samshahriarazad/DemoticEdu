<?php
// htdocs/demoticedu/backend/send_otp_email.php

declare(strict_types=1);

function demotic_send_otp(string $toEmail, string $otp): bool
{
  $cfg = require __DIR__ . '/mail_config.php';

  // DEV MODE: write OTP into file
  if (!empty($cfg['DEV_MODE'])) {
    $line = date('Y-m-d H:i:s') . " | {$toEmail} | OTP: {$otp}\n";
  ...
  if (!file_exists($autoload)) {
    error_log('OTP mail error: vendor/autoload.php not found.');
    return false;
  }
  ...
  try {
    ...
    if ($host === '' || $username === '' || $password === '' || $fromEmail === '') {
      error_log('OTP mail error: SMTP config is incomplete.');
      return false;
    }
    ...
    $mail->isHTML(true);
    $mail->Subject = 'DemoticEdu Email Verification OTP';
    ...
    $mail->Body = '
      <div style="font-family:Arial,Helvetica,sans-serif;max-width:560px;margin:0 auto;padding:24px;border:1px solid #e5e7eb;border-radius:12px;">
        ...
        <p style="margin:0 0 14px;color:#374151;line-height:1.6;">
          Use the OTP below to verify your DemoticEdu account:
        </p>
    ...
    $mail->AltBody = "Your DemoticEdu OTP is: {$otp}\n\nThis OTP will expire in 10 minutes.\n\nIf you did not request this, you can ignore this email.";
